...
export const ContentReviewTimelineModal: React.FC<ContentReviewTimelineModalProps> = ({
  ...
  const getActionBadge = (action: ReviewAction) => {
    switch (action) {
      ...
      case 'COMMENT_ADDED':
        return <Badge variant="neutral" size="sm">COMENTARIO</Badge>;
      default:
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Historial de revisión y gobernanza" size="lg">
      <div className="space-y-4">
        <div className="p-3.5 rounded-lg bg-[#161f30] border border-[#202b42] flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">Pista de auditoría cronológica e inmutable</p>
          </div>
        ...
        {loading ? (
          <div className="py-12 text-center text-xs text-slate-400 animate-pulse">
            Cargando eventos de gobernanza...
          </div>
        ) : events.length === 0 ? (
          <div className="py-12 text-center text-xs text-slate-500">
            Ningún evento de revisión registrado por el momento.
          </div>
        ) : (
